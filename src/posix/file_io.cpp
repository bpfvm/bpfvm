...
// 返回 true 表示已处理（已设好 r(0)，调用方应直接 return true）；false 表示放行真正 I/O。
bool PosixSyscall::tty_bg_check(vm* v, const std::shared_ptr<fd_handle>& fd, bool is_read) {
    const auto& tty = fd->tty;
    if(!tty || !session || session->ctty.get() != tty.get()) return false;
    uint64_t fg = tty->fg_pgrp.load(std::memory_order_acquire);
    if(fg == 0 || pgrp->pgid == fg) return false;  // 前台组：放行
    ...
    if(!is_read) {
        ...
        if(tcgetattr(fd->fd, &tio) == 0 && !(tio.c_lflag & TOSTOP)) {
            return false;  // TOSTOP=0：放行，do_write 正常执行
        }
...
bool PosixSyscall::do_openat(vm* v) {
    ...
    if(guest_abs == "/dev/ptmx") {
        ...
        if(grantpt(master) < 0 || unlockpt(master) < 0) {
            int e = errno; close(master); v->r(0) = -e; return true;
        }
        int ptn = -1;
    ...
    if(guest_abs.rfind("/dev/pts/", 0) == 0) {
        ...
        if(!tty) { v->r(0) = -ENOENT; return true; }
...
bool PosixSyscall::do_read(vm* v) {
    ...
    if(tty_bg_check(v, it->second, /*is_read=*/true)) {
        return true;  // 后台读 ctty：tty_bg_check 已投 SIGTTIN 并设好 r(0)
    }
...
bool PosixSyscall::do_write(vm* v) {
    ...
    if(tty_bg_check(v, it->second, /*is_read=*/false)) {
        return true;  // 后台写 ctty：tty_bg_check 已投 SIGTTOU 并设好 r(0)
    }
    size_t count = arg_size(v->r(3));
...
bool PosixSyscall::do_readv(vm* v) {
    ...
    if(tty_bg_check(v, it->second, /*is_read=*/true)) {
        return true;  // 后台读 ctty：已投 SIGTTIN
    }
