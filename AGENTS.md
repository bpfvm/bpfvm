...
- `root/`: unified install root for the C/C++ toolchain + rootfs. `root/include` (musl headers), `root/lib` (`libc.a`/`libc.so`/`ld-bpf.so`/`libcxx.a`/`libcxx.so`/...), `root/bin` (dash/busybox). Built by `sh musl/build.sh` + `./scripts/build_root.sh`.
...
- `./scripts/build_root.sh` — build demo rootfs (`dash` + `busybox`) and install to `root/bin` (requires `clang`, `gcc`, and `libelf`).
...
## Known LLVM BPF backend bugs (workarounds in this repo)
Upstream LLVM's BPF backend has several defects that this repo works around. For each: symptom → root cause → workaround → upstream issue. Other sections reference this one rather than repeating the details. (The 5-arg / no-FP / no-varargs limits in §1–2 are by-design architecture constraints, not bugs.)
