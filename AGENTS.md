...
*   **By-value struct parameters:** clang lowers C++ by-value aggregate parameters in two ways, and `lowerAggregateParams` normalizes both to a plain `ptr` (1 register each):
    ...
    *   **Small aggregates (≤2 words, e.g. `std::pair`=`{i64,i64}`, `__bit_iterator`=`[2 x i64]`, `i128`):** clang uses the aggregate **value type** directly as the parameter type (no `byval`). The BPF backend expands this into multiple registers per element/field, so `f(__bit_iterator, __bit_iterator, value, proj)` = 2+2+1+1 = 6 registers > the 5-register limit → "too many arguments". The pass rebuilds the signature to a plain `ptr`, moves the body, and rewrites the body's uses of the value parameter plus each call site's argument. Two sub-cases by whether the aggregate is trivially copyable:
        *   **Trivially copyable** (`std::pair` of scalars, `i128`, ...): callee entry inserts a `load T, ptr %arg`, call site stores the value into a fresh entry-block alloca and passes its pointer. Plain bitwise copy is correct.
        *   **Non-trivially copyable** (e.g. `std::shared_ptr` = `{ptr,ptr}` 16B, anything with a user-defined copy/move ctor or destructor): clang generates a `[2 x i64]` value parameter and a body of the form `store [2 x i64] %arg, ptr %local; ... move/copy %local ...`. A bitwise `load`/`store` would break move semantics — `move %local` nulls `%local` (the load copy), not the caller's source, so the caller's copy-constructed temporary is still destroyed and the refcount is decremented once too many (silent: refcount ends up one short, caller's `shared_ptr` dangles, next fork/access UAFs; reproduced by `test/test_sp_copy.cpp` and the `bpfvm-on-bpfvm` `test_waitid` crash, where `do_clone`'s `make_shared<PosixSyscall>(pgrp, session)` under-counted `pgrp`). Fix mirrors x86's invisible-reference ABI: `rewriteValueParamUsesToPointer` erases the `store %arg, ptr %local`, RAUWing `%local` with the pointer parameter so the body operates directly on the caller's source object; and the call site, when the argument is `load T, ptr %src` (clang's pattern: caller copy/move-constructs a temporary `%src`, then loads), passes `%src` itself so the callee's move nulls the caller's temporary and its destructor becomes a no-op. Non-load arguments (rare; e.g. a value returned by another call) fall back to the trivially-copyable alloca+store path.
