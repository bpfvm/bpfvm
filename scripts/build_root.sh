...
# 安装到 musl/build/install，经项目根的 libc 软链暴露（libc/include、libc/lib/libc.a）。
build_musl() {
    echo "Building musl..."
    cd "${ROOT_DIR}"
    sh musl/build.sh
...
# libc.so 而非 libc.a），由 musl/build.sh 单独编译成 dlstart.lo/dynlink.lo 安装到 libc/lib/。
#
# 只构建一份二进制——libc.a + ldso objects，SONAME=libc.so，入口 _dlstart。
#   - libc/lib/libc.so：这份二进制本体。链接时（-l:libc.so）读它的 dynsym（libc 符号
...
build_libc_bpfso() {
    ...
    "${BPFVM_LD}" -shared --soname libc.so -e _dlstart \
        ...
        -o "${ROOT_DIR}/libc/lib/libc.so"
    mkdir -p "${ROOT_DIR}/root/lib"
    # root/lib：ld-bpf.so 实文件，libc.so 相对 symlink 指向它。
    cp -f "${ROOT_DIR}/libc/lib/libc.so" "${ROOT_DIR}/root/lib/ld-bpf.so"
    ln -sf ld-bpf.so "${ROOT_DIR}/root/lib/libc.so"
}

# 构建 C++ 运行时（libcxx.a + libcxx.so）。
#   libcxx.a：由 scripts/build_libcxx.sh 编译 libc++/libc++abi 源码打成静态库，放到 libcxx/lib/。
#   libcxx.so：用 bpfvm-ld -shared 从 libcxx.a 合成（PIE ET_DYN），依赖 libc.so（DT_NEEDED），
#     仿 build_libc_bpfso，放到 libcxx/lib/ 并复制到 root/lib/（rootfs 与 ctest 运行时共用）。
...
build_libcxx() {
    ...
    "${BPFVM_LD}" -shared --soname libcxx.so \
        "${ROOT_DIR}/libcxx/lib/libcxx.a" \
        -L "${ROOT_DIR}/libc/lib" -l:libc.so \
        -o "${ROOT_DIR}/libcxx/lib/libcxx.so"
    mkdir -p "${ROOT_DIR}/root/lib"
    cp -f "${ROOT_DIR}/libcxx/lib/libcxx.so" "${ROOT_DIR}/root/lib/libcxx.so"
}
