...
//   1. 返回值非负（不是 -ENOSYS=-38 的负值）——验证 syscall 已实现。
//   2. uid==euid 且 gid==egid（无 setuid/setgid 差异 → OPENSSL_issetugid 返 0）。
//   3. getgroups 一致：getgroups(0)==n 后 getgroups(n,buf)==n（无补充组在 bpfvm 下 n==0）。
//
...
int main(void) {
    ...
    /* 1) 非 -ENOSYS：uid_t 是无符号，但 musl/bpfvm 下未实现时 raw 返回 -38，
